...
<style>
    ...
    #iw-container .iw-title {font-family: 'Open Sans Condensed', sans-serif;font-size: 15px;font-weight: 400;padding: 8px;background-color: #48b5e9;color: white;margin: 0;border-radius: 2px 2px 0 0;}
    #iw-container .iw-content {font-size: 13px;line-height: 18px;font-weight: 400;margin-right: 1px;padding: 5px 5px 5px 10px;max-height: 140px;overflow-y: auto;overflow-x: hidden;}
...
<script>
    ...
    function initMap() {
        ...
        map = new google.maps.Map(document.getElementById("vmap"), mapOptions);
...
if ($this->input->get('track_start_date')) {
    $track_start_date = date('d F, Y', strtotime($this->input->get('track_start_date')));
}
if ($this->input->get('track_start_time')) {
    $track_start_time = date('h:i A', strtotime($this->input->get('track_start_time')));
}
if ($this->input->get('track_end_date')) {
    $track_end_date = date('d F, Y', strtotime($this->input->get('track_end_date')));
}
if ($this->input->get('track_end_time')) {
    $track_end_time = date('h:i A', strtotime($this->input->get('track_end_time')));
}
?>